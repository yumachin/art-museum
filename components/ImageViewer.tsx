...
const ImageViewer: React.FC<ImageViewerProps> = ({ src, alt, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 backdrop-blur-sm animate-fade-in">
      <div className="absolute top-0 left-0 w-full pt-safe-top px-safe-x pb-4 flex justify-between items-start z-[80] pointer-events-none">
        <div className="pointer-events-auto bg-black/40 backdrop-blur-md rounded-full px-3 py-1 text-museum-ivory/80 text-xs font-serif uppercase tracking-widest border border-white/10">
          Viewer Mode
        </div>
        <button 
          onClick={onClose}
          className="pointer-events-auto p-2.5 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors backdrop-blur-md touch-manipulation"
        >
      ...
      <div className="absolute bottom-[calc(2rem+env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 flex gap-4 z-[80]">
        <button 
